  ...
  useEffect(() => {
    const getExamDetails = async () => {
      try {
        ...
        const formattedStartTime = new Date(startTime * 1000).toLocaleString(); // Format the date if it's in Unix timestamp
        const formattedLastEnrollmentDate = new Date(lastEnrollmentDate * 1000).toLocaleString(); // Format the last enrollment date
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-customYellow3 p-6">
      <div className="card bg-white shadow-lg rounded-lg w-96">
        {exam.examName ? (
          <div className="card-body">
            <h2 className="card-title text-center text-customYellow2">{exam.examName}</h2>
            <p className="text-md text-gray-700">Exam Date: {exam.startTime}</p>
            <p className="text-md text-gray-700">Last Date for Enrollment: {exam.lastEnrollmentDate}</p>
            <p className="text-md text-gray-700">Duration: {exam.duration} minutes</p>
